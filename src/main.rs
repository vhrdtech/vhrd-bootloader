...
use vhrd_flash_writer::flash::{
    flash_read, flash_read_slice, flash_size_bytes, stm32_device_signature, FlashWriter,
    FlashWriterError,
};
...
use vhrdcan::{Frame, FrameId, FrameRef};
//use stm32f0xx_hal::pac::syscfg::cfgr1::MEM_MODE_A::SRAM;
use crc::{Algorithm, Crc, CRC_32_AUTOSAR};

use core::convert::TryFrom;
use embedded_hal::digital::v2::OutputPin;
use uavcan_llr::tailbyte::{Kind, TailByte};
use uavcan_llr::types::{
    CanId, NodeId, Priority, Service, ServiceId, SubjectId, TransferId, TransferKind,
};
...
const DEFAULT_NODE_ID: NodeId = NodeId::new(4).unwrap();
...
fn can_transmit(can_iface: &mut CanInstance, frame_id: FrameId, data: &[u8]) {
    ...
    if data.is_empty() {
        ...
        let mut tail_byte = TailByte::new_multi_frame(
            tr_id,
            chunks.len() + if reminder.is_empty() { 0usize } else { 1usize },
        );
        ...
        for msg in chunks.into_iter() {
            ...
            can_iface
                .transmit(&BxFrame::new_data(
                    frame_id,
                    match BxData::new(can_data.as_slice()) {
                        None => {
                            panic!()
                        }
                        Some(d) => d,
        ...
        if !reminder.is_empty() {
            ...
            can_iface
                .transmit(&BxFrame::new_data(
                    frame_id,
                    match BxData::new(&can_data[0..reminder.len() + 1]) {
                        None => {
                            panic!()
                        }
                        Some(d) => d,
...
fn can_worker<'a, R: FnMut(CommandEvent, &NodeId, &NodeId) -> Option<(FrameId, &'a [u8])>>(
    can_iface: &mut CanInstance,
    src_node_id: &NodeId,
    mut r: R,
) -> Option<CanWorkerResult> {
    let mut cnt = 0u16;
    while cnt < 4000 {
        cnt += 1;
        #[cfg(feature = "spi-can")]
        let rx_frame_opt = if can_iface.interrupt_flags().rx0if_is_set() {
            Some(can_iface.receive(McpReceiveBuffer::Buffer0))
        } else {
            None
        };
        #[cfg(feature = "reg-can")]
        let rx_frame_opt = match can_iface.receive() {
            ...
            Ok(frame) => {
                if frame.is_data_frame() {
                    let rx_frame = match Frame::<8>::new(
                        bxcanid2vhrdcanid(frame.id()),
                            ...
                            if s.destination_node_id == *src_node_id {
                                if s.is_request {
                                    ...
                                    if rx_frame.data().len() != 0 {
                                        let tail_byte =
                                            TailByte::from(*rx_frame.data().last().unwrap());
                                        let data_transfer_state =
                                            if tail_byte.kind == Kind::MultiFrame {
                                                DataTransferState::StartOfTransfer
                                            } else if tail_byte.kind == Kind::EndT0
                                                || tail_byte.kind == Kind::EndT1
                                            {
                                        ...
                                        let res = match s.service_id {
                                            READ_SERVICE => {
                                                let read_option = if rx_frame.data()[0]
                                                    == READ_CONFIG_CMD
                                                    && rx_frame.data().len() == 2
                                                {
                                                    BootloaderReadOptions::Config
                                                } else if rx_frame.data()[0] == READ_FIRMWARE_CMD
                                                    && rx_frame.data().len() == 2
                                                {
                                                    BootloaderReadOptions::Firmware
                                                } else if rx_frame.data()[0] == READ_BOOTLOADER_CMD
                                                    && rx_frame.data().len() == 2
                                                {
                                                    BootloaderReadOptions::Bootloader
                                                } else {
                                                    BootloaderReadOptions::RawAddress
                                                };
                                                r(
                                                    CommandEvent::Read(
                                                        read_option,
                                                        &rx_frame.data()[0..rx_frame.data().len()],
                                                    ),
                                                    &s.destination_node_id,
                                                    &uavcan_id.source_node_id,
                                                )
                                            }
                                            WRITE_CONFIG_SERVICE => r(
                                                CommandEvent::Write(
                                                    BootloaderWriteOptions::Config,
                                                    data_transfer_state,
                                                    &rx_frame.data()
                                                        [0..(rx_frame.data().len() - 1)],
                                                ),
                                                &s.destination_node_id,
                                                &uavcan_id.source_node_id,
                                            ),
                                            WRITE_FIRMWARE_SERVICE => {
                                                if data_transfer_state
                                                    == DataTransferState::EndOfTransfer
                                                {
                                                    can_worker_res =
                                                ...
                                                r(
                                                    CommandEvent::Write(
                                                        BootloaderWriteOptions::Firmware,
                                                        data_transfer_state,
                                                        &rx_frame.data()
                                                            [0..(rx_frame.data().len() - 1)],
                                                    ),
                                            ...
                                            _ => r(
                                                CommandEvent::Error(CommandError::WrongServiceId),
                                                &s.destination_node_id,
                                                &uavcan_id.source_node_id,
                                            ),
                                        };
                                        ...
                                        let res = r(
                                            CommandEvent::Error(CommandError::NoCanData),
                                            &s.destination_node_id,
                                            &uavcan_id.source_node_id,
                                        );
                                        match res {
...
fn main() -> ! {
    ...
    #[cfg(feature = "stm32f0xx-hal")]
    let mut nv_config_range = match FlashWriter::new(
        NV_CONFIG_START_ADDR as u32..(NV_CONFIG_START_ADDR as u32 + SIZE_OF_NVCONFIG as u32),
    ) {
        Ok(f) => f,
        Err(_) => {
            panic!()
    ...
    let mut firmware_range = match FlashWriter::new(
        (NV_CONFIG_START_ADDR as u32 + SIZE_OF_NVCONFIG as u32)..(0x0800_0000 + flash_size_bytes()),
    ) {
        Ok(f) => f,
        Err(_) => {
            panic!()
    ...
    #[cfg(feature = "stm32f4xx-hal")]
    let mut firmware_range = match FlashWriter::new(0x0800_C000u32..0x080E_0000u32) {
        Ok(f) => f,
        Err(_) => {
            panic!()
        }
    ...
    loop {
        ...
        state = match state {
            State::CheckNVConfig => {
                ...
                let res = match get_crc(flash_read_slice::<u8>(
                    NV_CONFIG_START_ADDR as u32 + 8u32,
                    SIZE_OF_NVCONFIG - 8,
                )) == nv_config.config_crc
                {
                    true => {
                        node_id =
                            unsafe { NodeId::new_unchecked(nv_config.board_config.uavcan_node_id) };
                ...
                let filter =
                    (1u32 << 25) | ((node_id.inner() as u32) << 7) | FLASHER_NODE_ID.inner() as u32;
                cfg_if! {
            ...
            State::CheckBootloaderValidity => {
                ...
                if nv_config.board_config.bootloader_size + 0x0800_0000
                    <= NV_CONFIG_START_ADDR as u32
                    && nv_config.board_config.bootloader_size != 0
                {
                    match get_crc(flash_read_slice::<u8>(
                        0x0800_0000,
                        nv_config.board_config.bootloader_size as usize,
                    )) == nv_config.board_config.bootloader_crc
                    {
            ...
            State::CheckForFirmware => {
                ...
                if nv_config.board_config.fw_size > 0
                    && nv_config.board_config.fw_size
                        < (flash_size_bytes() - SIZE_OF_NVCONFIG as u32 - 10u32.kb())
                {
            ...
            State::CheckFirmwareValidity => {
                ...
                match get_crc(flash_read_slice::<u8>(
                    firmware_range.get_start_address(),
                    nv_config.board_config.fw_size as usize,
                )) == nv_config.board_config.fw_crc
                {
                    true => State::WaitingNewCommandTimeout,
                    false => {
                ...
                match timeout_limit <= (pass_time_us / 1000) as u16 {
                    ...
                    false => {
                        match can_worker(
                            p.5.borrow_mut(),
                            node_id.borrow_mut(),
                            |command, src_node_id, dst_node_id| {
                                match command {
                                    CommandEvent::Read(br, data) => {
                                        ...
                                        let res_slice = match br {
                                            ...
                                            BootloaderReadOptions::Bootloader => {
                                                flash_read_slice::<u8>(0x0800_0000u32, 10.kb())
                                            }
                                        ...
                                        Some((
                                            match FrameId::new_extended(unsafe {
                                                CanId::new_service_kind(
                                                    *src_node_id,
                                                    *dst_node_id,
                                                    READ_SERVICE,
                                                    false,
                                    ...
                                    CommandEvent::Write(bw, tr, data) => {
                                        let (mut flash_region, service_id) = match bw {
                                            BootloaderWriteOptions::Config => {
                                                (nv_config_range.borrow_mut(), WRITE_CONFIG_SERVICE)
                                            }
                                            BootloaderWriteOptions::Firmware => (
                                                firmware_range.borrow_mut(),
                                                WRITE_FIRMWARE_SERVICE,
                                            ),
                                        };
                                        match tr {
                                            DataTransferState::StartOfTransfer => {
                                                ...
                                                flash_region
                                                    .write(flash_regs.borrow_mut(), data)
                                                    .ok();
                                            }
                                            DataTransferState::DataTransfer => {
                                                flash_region
                            ...
                            Some(r) => match r {
                                CanWorkerResult::FirmwareReceived => State::Reboot,
                                _ => {
                                    pass_time_us = 0;
                                    State::WaitingCommand
                                }
            ...
            State::Boot => {
                can_transmit(
                    p.5.borrow_mut(),
                    FrameId::new_extended(
                        CanId::new_message_kind(node_id, BOOT_MSG, false, Priority::High).into(),
                    )
                    .unwrap(),
                    &[],
                );
                ...
                unsafe {
                    ...
                    core::ptr::copy_nonoverlapping::<u32>(
                        firmware_range.get_start_address() as *const u32,
            ...
            State::Reboot => {
                ...
                can_transmit(
                    p.5.borrow_mut(),
                    FrameId::new_extended(
                        CanId::new_message_kind(node_id, REBOOT_MSG, false, Priority::High).into(),
                    )
                    .unwrap(),
                    &[],
                );
...
fn panic(_info: &PanicInfo) -> ! {
    ...
    // match info.location() {
    //     Some(location) => {
    //         rprintln!("Panic: l: {} col: {}", location.line(), location.column());
    //     },
    //     None => {
